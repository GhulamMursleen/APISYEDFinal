            ...
            if len(contain_values)>0:
                 ...
                 for index, row in rows.iterrows():
                     if (len(data)<10):
                         ...
                         data.append({"description":row["description"],"clause_type":row["name"],"category":claname[claid.index(row["clauseID"])],"tag":row["tags"]})
                 #print("data",data)
                 if len(data)==0:
                 ...
                 for x in results:
                     if (len(data)<n):
                         ...
                         data.append({"description":descriptions[claid.index(x[1])],"clause_type":entities[claid.index(x[1])],"category":claname[claidss[claid.index(x[1])]],"tag":tagss[claid.index(x[1])]})
                     else:
                         break
                 #print("data",data)
                 array2=[]
...
def merge():
   ...
   if request.method == "OPTIONS": # CORS preflight
        return _build_cors_prelight_response()
                ...
                array=[]
                for index, row in df.iterrows() :
                        array.append({"id":row[0],"contractType":row[1]})
                ...
                for index, row in df.iterrows() :
                        ...
                        array2.append({"id":row[0],"clausecategory":row[1]})
                ...
                #############################mergeall
                result={"clauseCategory":array,"contractType":array2,"tag":array3}
                #print(merge)
